...
test("return HTMLWebpackPlugin", () => {
  const htmlWebpackPlugins = entriesGenerator.buildHtmlWebpackPlugins({"abc": "abc.js"}, ".");
  expect(htmlWebpackPlugins[0].userOptions.filename).toBe("abc.html");
  expect(htmlWebpackPlugins[0].userOptions.chunks[0]).toBe("abc");
  expect(path.basename(htmlWebpackPlugins[0].userOptions.template)).toBe("abc.html");
});